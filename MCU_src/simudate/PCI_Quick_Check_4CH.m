%% PCI四通道数据快速预览与合格性检查
% 使用方法：将本文件放在PCI数据目录，只修改下面“用户配置区”，
% 然后在MATLAB中直接按“运行”。不需要命令行参数。
...
%% =========================== 用户配置区 ===========================
% 数据目录。默认为本脚本所在目录。
dataDirectory = string(fileparts(mfilename('fullpath')));
...
headerBytes = 1024;
previewStartSample = 1;       % 每通道的起始点，从1开始
previewSampleCount = 2e6;     % 2e6点=32 ms@62.5 MHz，读取仍很快
...
validateattributes(previewStartSample, {'numeric'}, ...
    {'scalar', 'integer', 'positive'});
validateattributes(previewSampleCount, {'numeric'}, ...
    {'scalar', 'integer', 'positive'});
...
actualRequestedCount = min(previewSampleCount, ...
    totalSamplesPerChannel - previewStartSample + 1);
...
fprintf('读取范围：%d .. %d（%.3f ms）\n', ...
    previewStartSample, previewStartSample + actualRequestedCount - 1, ...
