...
assert(contains(core_text, '.P_PRODUCT_SHIFT(8)'));
...
assert(contains(arm_text, 'DPLL_CORDIC_HEADROOM_BITS   2U'));
assert(contains(arm_text, '{  30000U,  4000U, 8000U, 0U }'));
assert(contains(arm_text, 'profile->kp_blend = 375000'));
...
table_value = table( ...
    ...
    ["P_PRODUCT_SHIFT=8"; ...
     ...
     "20 kHz band selects R16/shift9/TRACK8k/Kp6M; panel supplies Ki"], ...
...
fprintf(file_id, ['“RTL复位检测链”只表示wrapper的R31/shift10/8k字面值，' ...
    '不冒充ARM加载后的工作profile；“更新前ARM工作profile”保留' ...
    'R16/shift8/2k/Ki180k历史负对照。当前实验代码使用P8、' ...
    'R16/shift9和8 kHz TRACK，Kp/Ki由面板设置。\n\n']);
