...
$OutDir = Join-Path $RepoRoot "reports\xsim\debug_dac_formatter_stage_a_cli_$RunStamp"

New-Item -ItemType Directory -Force $OutDir | Out-Null
Push-Location $OutDir

try {
    $snapshot = 'debug_dac_formatter_stage_a_tb'
    $rtl = @(
        Join-Path $RepoRoot 'DPLL_Rewrite.srcs\sources_1\DigitalPLL\debug\debug_dac_formatter_stage_a.v'
        Join-Path $RepoRoot 'verification\rtl\debug_dac_formatter_stage_a_tb.v'
    )
    ...
    & (Join-Path $VivadoBin 'xelab.bat') debug_dac_formatter_stage_a_tb -snapshot $snapshot 2>&1 |
    ...
    if ($content -notmatch 'PASS: debug_dac_formatter_stage_a_tb') { throw 'missing PASS marker' }
}
finally {
    Pop-Location
}
