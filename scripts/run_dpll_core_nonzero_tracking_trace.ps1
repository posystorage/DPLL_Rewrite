...
$OutDir = Join-Path $RepoRoot "reports\xsim\dpll_core_nonzero_tracking_$RunStamp"

New-Item -ItemType Directory -Force $OutDir | Out-Null
Push-Location $OutDir

try {
    $snapshot = 'dpll_core_nonzero_tracking_tb'
    $vhdl = @(
    ...
    $rtl = @(
        ...
        Join-Path $RepoRoot 'verification\rtl\dpll_core_nonzero_tracking_tb.v'
    )
    ...
    if ($content -notmatch 'PASS: dpll_core_nonzero_tracking_tb') { throw 'missing PASS marker' }

    $trace = Join-Path $OutDir 'dpll_core_nonzero_tracking_trace.csv'
    python (Join-Path $RepoRoot 'verification\fixed_point\check_dpll_core_nonzero_tracking_trace.py') $trace
    if ($LASTEXITCODE -ne 0) { throw "DPLL core nonzero tracking checker failed with exit code $LASTEXITCODE" }
}
finally {
    Pop-Location
}
