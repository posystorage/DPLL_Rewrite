...
try {
    ...
    $rtl = @(
        ...
        Join-Path $RepoRoot 'verification\rtl\dpll_multifrequency_path_tb.v'
    )
    ...
    if ($content -notmatch 'PASS: dpll_multifrequency_path_tb') { throw 'missing PASS marker' }
    foreach ($case in 0..6) {
        if ($content -notmatch "PASS_CASE: index=$case") {
