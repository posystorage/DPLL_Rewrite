...
$OutDir = Join-Path $RepoRoot "reports\xsim\fll_cross_dot_stage_a_cli_$RunStamp"

New-Item -ItemType Directory -Force $OutDir | Out-Null
Push-Location $OutDir

try {
    $snapshot = 'fll_cross_dot_stage_a_tb'
    $rtl = Join-Path $RepoRoot 'DPLL_Rewrite.srcs\sources_1\DigitalPLL\hybrid_loop\fll_phase_difference_stage_a.v'
    $tb = Join-Path $RepoRoot 'verification\rtl\fll_cross_dot_stage_a_tb.v'

    & (Join-Path $VivadoBin 'xvlog.bat') $rtl $tb
    if ($LASTEXITCODE -ne 0) { throw "xvlog failed with exit code $LASTEXITCODE" }

    $xelabLog = Join-Path $OutDir 'xelab_stdout.log'
    & (Join-Path $VivadoBin 'xelab.bat') fll_cross_dot_stage_a_tb -snapshot $snapshot 2>&1 |
    ...
    & (Join-Path $VivadoBin 'xsim.bat') $snapshot -runall -log fll_cross_dot_stage_a_xsim.log
    if ($LASTEXITCODE -ne 0) { throw "xsim failed with exit code $LASTEXITCODE" }

    $content = Get-Content -LiteralPath (Join-Path $OutDir 'fll_cross_dot_stage_a_xsim.log') -Raw
    if ($content -match 'FAIL:') { throw 'xsim reported FAIL' }
    if ($content -notmatch 'PASS: fll_cross_dot_stage_a_tb') { throw 'missing pass marker' }
}
finally {
    Pop-Location
}
