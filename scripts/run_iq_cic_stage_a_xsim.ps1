...
$OutDir = Join-Path $RepoRoot "reports\xsim\iq_cic_stage_a_cli_$RunStamp"

New-Item -ItemType Directory -Force $OutDir | Out-Null
Push-Location $OutDir

try {
    $snapshot = 'post_iq_cic_stage_a_tb'
    ...
    & (Join-Path $VivadoBin 'xelab.bat') post_iq_cic_stage_a_tb -snapshot $snapshot 2>&1 |
    ...
    & (Join-Path $VivadoBin 'xsim.bat') $snapshot -runall -log post_iq_cic_stage_a_xsim.log
