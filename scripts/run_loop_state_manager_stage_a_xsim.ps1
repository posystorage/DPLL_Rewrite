...
$OutDir = Join-Path $RepoRoot "reports\xsim\loop_state_manager_stage_a_cli_$RunStamp"

New-Item -ItemType Directory -Force $OutDir | Out-Null
Push-Location $OutDir

try {
    $snapshot = 'loop_state_manager_stage_a_tb'
    $rtl = @(
        Join-Path $RepoRoot 'DPLL_Rewrite.srcs\sources_1\DigitalPLL\hybrid_loop\loop_state_manager_stage_a.v'
        Join-Path $RepoRoot 'verification\rtl\loop_state_manager_stage_a_tb.v'
    )
    ...
    & (Join-Path $VivadoBin 'xelab.bat') loop_state_manager_stage_a_tb -snapshot $snapshot 2>&1 |
    ...
    if ($content -notmatch 'PASS: loop_state_manager_stage_a_tb') { throw 'missing PASS marker' }
}
finally {
    Pop-Location
}
