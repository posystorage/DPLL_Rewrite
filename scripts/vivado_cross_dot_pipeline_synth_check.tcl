...
set out_dir [file join $repo_root reports vivado_cross_dot_pipeline_synth_check]
...
synth_design -top fll_cross_dot_stage_a -mode out_of_context
create_clock -period 8.000 -name clk_125m [get_ports clk_125m]

set util_file [file join $out_dir cross_dot_utilization_ooc.rpt]
set timing_file [file join $out_dir cross_dot_timing_ooc.rpt]
...
puts "CROSS_DOT_DSP_COUNT=$dsp_count"
puts "CROSS_DOT_WNS=$wns"
if {$dsp_count > 4} {
    error "fll_cross_dot_stage_a uses $dsp_count DSP48E1 cells; limit is 4"
}
if {$wns < 0.0} {
    error "fll_cross_dot_stage_a timing WNS is $wns ns"
}
close_project
