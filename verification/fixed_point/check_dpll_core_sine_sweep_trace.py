...
def check_case(case_index: int, rows: list[dict[str, str]]) -> tuple[list[str], str]:
    ...
    if (input_hz - center_hz) * direction <= 0:
        failures.append(f"input_hz {input_hz} has wrong direction around center_hz {center_hz}")
    ...
    for index, row in enumerate(rows):
        ...
        if correction != 0:
            nonzero += 1
        if (tracking_word - center_word) * direction > 0:
            directional += 1
            if index >= tail_start:
                tail_directional += 1
    ...
    if directional < 4:
        failures.append(f"directional tracking rows {directional} < 4")
    if tail_directional < max(4, len(rows) - tail_start - 1):
        failures.append(f"tail directional tracking rows {tail_directional} too low for {len(rows) - tail_start} tail rows")
    ...
    line = (
        ...
        f"`0x{expected_delta:012x}` | {len(rows)} | {nonzero} | {directional} | "
        f"{tail_directional}/{len(rows) - tail_start} | {control_model_matches}/{len(rows)} | "
...
def check_rows(rows: list[dict[str, str]]) -> tuple[list[str], list[str]]:
    ...
    expected_cases = 2 * len(EXPECTED_CENTERS)
    if sorted(grouped) != list(range(expected_cases)):
        failures.append(f"expected case indexes 0..{expected_cases-1}, got {sorted(grouped)}")

    for case_index in range(2 * len(EXPECTED_CENTERS)):
        expected_center = EXPECTED_CENTERS[case_index % len(EXPECTED_CENTERS)]
