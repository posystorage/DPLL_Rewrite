...
module dpll_single_clock_core_stage_a(
 input clk_125m,input rst_125m,input sample_valid,input loop_enable,input signed [15:0] adc_sample,input [47:0] center_word,input config_apply,input [8:0] cic_rate_r,input [5:0] cic_output_shift,input cic_flush,input [1:0] fll_delay_sel,input [1:0] post_iir_mode,input signed [31:0] post_iir_acq_b0,input signed [31:0] post_iir_acq_b1,input signed [31:0] post_iir_acq_b2,input signed [31:0] post_iir_acq_a1,input signed [31:0] post_iir_acq_a2,input signed [31:0] post_iir_track_b0,input signed [31:0] post_iir_track_b1,input signed [31:0] post_iir_track_b2,input signed [31:0] post_iir_track_a1,input signed [31:0] post_iir_track_a2,input signed [23:0] kf,input signed [23:0] ki,input signed [23:0] kp,input signed [23:0] kf_blend,input signed [23:0] kf_track,input signed [23:0] kp_blend,input signed [23:0] ki_blend,input signed [17:0] phase_setpoint,input [17:0] phase_lock_threshold,input [21:0] freq_lock_threshold,input [19:0] mag_enter_threshold,input [19:0] mag_exit_threshold,input [15:0] acquire_dwell,input [15:0] blend_dwell,input [15:0] loss_dwell,input [23:0] measurement_timeout,input [23:0] holdover_timeout,input [15:0] warmup_samples,input signed [55:0] positive_limit,input signed [55:0] negative_limit,
output reg [47:0] tracking_word,output tracking_valid,output signed [17:0] cordic_phase_out,output signed [17:0] phase_error,output signed [21:0] freq_error,output freq_error_valid,output signed [19:0] i_baseband,output signed [19:0] q_baseband,output iq_valid,output signed [55:0] freq_state,output signed [55:0] freq_correction,output [19:0] magnitude,output [3:0] loop_state,output [3:0] loss_reason,output signal_present,output phase_locked,output frequency_locked,output locked,output [8:0] active_cic_rate_r,output [5:0] active_cic_output_shift,output post_iir_active_bypass,output post_iir_active_use_track,output cic_overflow_seen,output cic_illegal_config_seen,output cordic_input_overrun_seen,output cordic_input_out_of_range_seen,output cordic_output_format_error_seen,output signed [15:0] lo_cos,output signed [15:0] lo_sin);
reg [31:0] apply_count; initial begin tracking_word=0; apply_count=0; end always @(posedge clk_125m) if(config_apply) begin tracking_word<=center_word; apply_count<=apply_count+1; end assign tracking_valid=config_apply; assign cordic_phase_out=0; assign phase_error=0; assign freq_error=0; assign freq_error_valid=0; assign i_baseband=0; assign q_baseband=0; assign iq_valid=0; assign freq_state=0; assign freq_correction=0; assign magnitude=0; assign loop_state=0; assign loss_reason=0; assign signal_present=0; assign phase_locked=0; assign frequency_locked=0; assign locked=0; assign active_cic_rate_r=cic_rate_r; assign active_cic_output_shift=cic_output_shift; assign post_iir_active_bypass=(post_iir_mode==2'd0); assign post_iir_active_use_track=(post_iir_mode==2'd2); assign cic_overflow_seen=0; assign cic_illegal_config_seen=0; assign cordic_input_overrun_seen=0; assign cordic_input_out_of_range_seen=0; assign cordic_output_format_error_seen=0; assign lo_cos=0; assign lo_sin=0; endmodule
...
module dpll_wrapper_cdc_tb;
 ...
 function [31:0] expected_active_crc; input dummy; reg [31:0] crc; begin
   ...
   crc=crc_mix(crc,32'h02e96a90); crc=crc_mix(crc,32'h05d2d51f); crc=crc_mix(crc,32'h02e96a90);
   crc=crc_mix(crc,32'habfe403c);
   expected_active_crc=crc_mix(crc,32'h1fa76a03);
 end endfunction
 reg [31:0] v; reg [47:0] saved_center; reg [31:0] saved_crc; reg [31:0] saved_apply_count; reg [3:0] saved_loop_state;
 initial begin repeat(4)@(posedge clk1); rst=1;sys_rstn=1;repeat(4)@(posedge sys_clk);
   ...
   wr(16'h0042,32'h00000007);repeat(8)@(posedge clk1);rd(16'h006f,v);if(v[15:8]!==1||v[0]||v[1]||dut.live_debug_dac_source!==32'h00000007||dut.dpll_single_clock_core_stage_a_inst.apply_count!==saved_apply_count||dut.dpll_loop_state!==saved_loop_state)begin $display("FAIL: debug source write disturbed apply/loop status=%h live=%h apply_count=%0d saved=%0d loop=%0d saved_loop=%0d",v,dut.live_debug_dac_source,dut.dpll_single_clock_core_stage_a_inst.apply_count,saved_apply_count,dut.dpll_loop_state,saved_loop_state);$finish;end
   rd(16'h011e,v);if(v!==saved_crc||dut.active_center_word!==saved_center)begin $display("FAIL: debug source changed active config crc=%h saved=%h center=%h",v,saved_crc,dut.active_center_word);$finish;end
